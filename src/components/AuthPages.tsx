...
function GolaineLogo({ size = "md" }: { size?: "sm" | "md" }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div
        className={`${iconSize} rounded-xl flex items-center justify-center bg-gradient-to-br from-[rgb(37,211,102)] to-[rgb(22,163,74)] shadow-lg shadow-[rgb(37,211,102)]/20`}
      >
        <svg
          width={svgSize}
          height={svgSize}
    ...
    >
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm text-[rgb(148,163,184)] hover:text-white transition-colors mb-8 group"
        >
        ...
        {/* Footer link */}
        <p className="text-center mt-6 text-sm text-[rgb(148,163,184)]">
          Déjà un compte ?{" "}
          <button
            onClick={() => setPageView("login")}
            className="text-[rgb(37,211,102)] font-medium hover:underline cursor-pointer"
          >
            Se connecter
          </button>
        </p>
...
export default function AuthPages() {
  ...
  return (
    <AnimatePresence mode="wait">
      {pageView === "signup" && <SignUpPage key="signup" />}
      {pageView === "login" && <LoginPage key="login" />}
    </AnimatePresence>
  );
}
